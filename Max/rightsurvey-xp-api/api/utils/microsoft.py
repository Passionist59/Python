...
class Client:
    ...
    def parse_response(self, response):
        ...
        if response.status_code == 204 or response.status_code == 201:
            if "OData-EntityId" in response.headers:
                response_url = response.headers["OData-EntityId"].split("/")
                responseid = re.search(r'\((.*?)\)', response_url[-1]).group(1)
                return responseid
            return True
    ...
    def create_contact(self, contact):
        return self._post('contacts', json=contact)
    ...
    def create_incident(self, incident):
        return self._post('incidents', json=incident)

    def update_incident(self, id, incident):
        if id != "":
            url = 'incidents({0})'.format(id)
            return self._patch(url, json=incident)
        raise Exception("To update an incident, necessitates the ID")

    def close_incident(self, incident):
        return self._post('CloseIncident', json=incident)

    def delete_incident(self, id):
        if id != "":
            return self._delete('incidents({0})'.format(id))
        raise Exception("To delete a campaign is necessary the ID")
